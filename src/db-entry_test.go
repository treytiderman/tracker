...
func Test_Get_All_Entries(t *testing.T) {
	_test_Reset_Entry_Database(t)
	...
	// Test Start
	entries, err := Get_All_Entries(db_test)
	if err != nil {
		t.Error(err)
	}
	...
	if entries[8].Notes != "Entry 1" {
		t.Errorf("got %s, expected %s", entries[8].Notes, "Entry 1")
	}

	if entries[8].Tracker_Id != 1 {
		t.Errorf("got %d, expected %d", entries[8].Tracker_Id, 1)
	}
...
func Test_Get_Log(t *testing.T) {
	...
	Add_Option_to_Field(db_test, money_card_id, 3, "American Express")
	money_entry_1, _ := Create_Entry(db_test, money_id, "9.99 dollars entered as 999")
	log1_id, _ := Add_Log_To_Entry(db_test, money_entry_1, money_amount_id, -999)
	log2_id, _ := Add_Log_To_Entry(db_test, money_entry_1, money_card_id, 1)
	money_entry_2, _ := Create_Entry(db_test, money_id, "not for what you think")
	Add_Log_To_Entry(db_test, money_entry_2, money_amount_id, -42069)
	Add_Log_To_Entry(db_test, money_entry_2, money_card_id, 3)
	money_entry_3, _ := Create_Entry(db_test, money_id, "big spendin")
	...
	// Test Start
	log1, err := Get_Log(db_test, log1_id)
	if err != nil {
		t.Error(err)
	}

	// s, _ := json.MarshalIndent(log1, "", "    ")
	// fmt.Println("JSON:", string(s))

	if log1.Id != 1 {
		t.Errorf("got %d, expected %d", log1.Id, 1)
	}

	if log1.Value != -999 {
		t.Errorf("got %d, expected %d", log1.Value, -999)
	}

	if log1.Present != "-9.99" {
		t.Errorf("got %s, expected %s", log1.Present, "-9.99")
	}

	log2, err := Get_Log(db_test, log2_id)
	if err != nil {
		t.Error(err)
	}

	// s2, _ := json.MarshalIndent(log2, "", "    ")
	// fmt.Println("JSON:", string(s2))

	if log2.Id != 2 {
		t.Errorf("got %d, expected %d", log2.Id, 1)
	}

	if log2.Value != 1 {
		t.Errorf("got %d, expected %d", log2.Value, 1)
	}

	if log2.Present != "Discover" {
		t.Errorf("got %s, expected %s", log2.Present, "Discover")
	}
